...
(setq debug-on-error nil)  ; Disable the backtrace on error
(setq debug-on-quit nil)   ; Disable the backtrace when quitting
...
;; Set fill column to 71 and enable auto-fill-mode
(setq-default fill-column 71)
(add-hook 'text-mode-hook #'auto-fill-mode)

;; Set fill column to 71 characters and enable auto-fill-mode for .txt files
(add-hook 'text-mode-hook
          (lambda ()
            (setq fill-column 71)
            (auto-fill-mode 1)))

;; Define a function to fill the current region or the entire buffer
(defun my-fill-text ()
  "Fill the current region or the entire buffer to fit within `fill-column`."
  (interactive)
  (if (use-region-p)
      (fill-region (region-beginning) (region-end))
    (fill-region (point-min) (point-max))))

;; Bind the function to C-c o f
(global-set-key (kbd "C-c o f") #'my-fill-text)
