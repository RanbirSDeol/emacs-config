...
let ConfigurationService = class ConfigurationService {
    ...
    async getDocumentSettings(uri) {
        ...
        if (options.suggestions != null) {
            ...
            for (const [key, value] of Object.entries(options.suggestions)) {
                if (value == null)
                    continue; // ignore default values.
                const property = (key.slice(0, 1).toLocaleLowerCase() +
                    key.slice(1));
                (_a = (_b = options.suggestionCategories)[property]) !== null && _a !== void 0 ? _a : (_b[property] = value ? 'on' : 'off');
            }
        ...
        __classPrivateFieldSet(this, _DocumentService_documents, createTextDocuments({
            create(uri, languageId, version, content) {
                const document = new GrammarlyDocument(vscodeLanguageserverTextdocument.TextDocument.create(uri, languageId, version, content), async () => {
                    ...
                    if (options.documentDialect === 'auto-text')
                        options.documentDialect = 'american';
                    ...
                    try {
                        ...
                        connection.console.error('[Error in Grammarly SDK]: ' + error.message + ' ' + error.stack);
                        throw error;
                    }
                });
